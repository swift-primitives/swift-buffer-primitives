...
| **[rawlayout-minimal-reproducer](rawlayout-minimal-reproducer/)** | Standalone minimal reproducer: Bug 1 REPRODUCES (3-module chain, 2+ cross-module @_rawLayout fields), Bug 2 does NOT reproduce (7 patterns tried — missing ~Escapable ingredient) | 2026-03-21 | Swift 6.2.4 | Bug 1: CONFIRMED, Bug 2: REFUTED (see copypropagation-nonescapable-mark-dependence) |
| **[copypropagation-nonescapable-mark-dependence](copypropagation-nonescapable-mark-dependence/)** | **Bug 2 standalone reproducer**: ~Escapable + @_lifetime(borrow) view type yields double end_lifetime in CopyPropagation. 4 variants tested: V1 crash, V2 fix (drop ~Escapable), V3 fix (@_optimize(none) on _read accessor), V4 no fix (init-level @_optimize(none) insufficient) | 2026-03-22 | Swift 6.2.4 | CONFIRMED (bug) + CONFIRMED (fix) |
...
| **[rawlayout-access-level-trigger](rawlayout-access-level-trigger/)** | Access level is an independent trigger: `internal` builds, `public`/`package` crash. **Canary for compiler fix — when this passes with `public`, all 22 `_deinitWorkaround` sites can be removed.** | 2026-03-21 | Swift 6.2.4 / 6.4-dev | CONFIRMED |

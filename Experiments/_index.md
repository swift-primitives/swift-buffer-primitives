...
| **[rawlayout-llvm-verifier-crash](rawlayout-llvm-verifier-crash/)** | **Consolidated**: LLVM verifier crash — 8 variants (V01-V08) covering baseline, struct-body threshold, extension-file, cross-module, class-ref, wrapper patterns, ~Copyable elements, pre-compiled deinit | 2026-03-21 | Swift 6.2.4 | CONFIRMED (bug) — #86652 STILL BROKEN in Swift 6.3; workaround required |
| **[rawlayout-sil-ownership-crash](rawlayout-sil-ownership-crash/)** | **Consolidated**: SIL ownership crash + enum _modify — 3 variants (V01-V03) covering CopyPropagation, enum _modify limitation, recovery strategies. **SUPERSEDED** for Bug 2 by root cause fix (removed ~Escapable from Property.View, 2026-03-22) | 2026-03-21 | Swift 6.2.4 | SUPERSEDED (Bug 2 fixed) |
| **[rawlayout-deinit-alternatives](rawlayout-deinit-alternatives/)** | **Consolidated**: deinit workaround alternatives — 4 variants (V01-V04) covering discard self, guard idempotence, escapable lifetime, slab bitmap cleanup | 2026-03-21 | Swift 6.2.4 | CONFIRMED — alternatives still relevant in Swift 6.3 (#86652 unfixed) |
| **[rawlayout-minimal-reproducer](rawlayout-minimal-reproducer/)** | Standalone minimal reproducer: Bug 1 REPRODUCES (3-module chain, 2+ cross-module @_rawLayout fields), Bug 2 does NOT reproduce (7 patterns tried — missing ~Escapable ingredient) | 2026-03-21 | Swift 6.2.4 | Bug 1: CONFIRMED (still in 6.3), Bug 2: REFUTED (fixed in 6.3) |
| **[copypropagation-nonescapable-mark-dependence](copypropagation-nonescapable-mark-dependence/)** | **Bug 2 standalone reproducer**: ~Escapable + @_lifetime(borrow) view type yields double end_lifetime in CopyPropagation. 4 variants tested: V1 crash, V2 fix (drop ~Escapable), V3 fix (@_optimize(none) on _read accessor), V4 no fix (init-level @_optimize(none) insufficient) | 2026-03-22 | Swift 6.2.4 | SUPERSEDED — #88022 fixed in Swift 6.3; 149 @_optimize(none) sites removed |
| [noncopyable-nested-deinit-chain](noncopyable-nested-deinit-chain/) | 3-package nested ~Copyable deinit chain reproducer (swiftlang/swift#86652). Moved from swift-institute. | 2026-03-21 | Swift 6.2.4 | CONFIRMED (production only) — #86652 still reproducing in Swift 6.3 |
| **[rawlayout-access-level-trigger](rawlayout-access-level-trigger/)** | Access level is an independent trigger: `internal` builds, `public`/`package` crash. **Canary for compiler fix — when this passes with `public`, all 22 `_deinitWorkaround` sites can be removed.** | 2026-03-21 | Swift 6.2.4 / 6.4-dev | CONFIRMED — Canary UNCHANGED in Swift 6.3; 22 workaround sites remain |

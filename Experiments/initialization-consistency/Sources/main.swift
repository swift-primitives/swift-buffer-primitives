// MARK: - Storage.Initialization Consistency Verification
// Purpose: Verify Storage.Initialization (.empty, .one, .two) transitions
...
//
// Hypothesis: Storage.Initialization state is always consistent with actual
...
func idx(_ n: UInt) -> Index<Storage> {
    Index<Storage>(Ordinal(n))
}

func cnt(_ n: UInt) -> Index<Storage>.Count {
    Index<Storage>.Count(Cardinal(n))
}

func slotCap(_ storage: Storage.Heap<Int>) -> UInt {
    storage.slotCapacity.rawValue.rawValue
}

/// Verifies that storage.initialization matches expected.
func verify(
    _ storage: Storage.Heap<Int>,
    matches expected: Storage.Initialization,
    label: String
) {
...
do {
    ...
    let storage = Storage.Heap<Int>.create(minimumCapacity: cnt(8))
...
do {
    ...
    let storage = Storage.Heap<Int>.create(minimumCapacity: cnt(4))
    let cap = slotCap(storage)
    print("  Actual capacity: \(cap)")
    var head: UInt = 0
    var count: UInt = 0

    func ringInit() -> Storage.Initialization {

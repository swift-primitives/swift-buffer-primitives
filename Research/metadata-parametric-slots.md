...
normative: false
upstream: hash-table-storage-buffer-layering.md (v2.0.0, Workstream 3)
changelog:
...
|---|----------|--------|-------|
| OQ-1 | Should `Buffer.Slots.Fixed.deinit` automatically deinitialize payloads if `Payload: BitwiseCopyable`? | OPEN | For trivial types, deinit is no-op anyway. For non-trivial `Copyable` types, auto-deinit is safe but requires knowing which slots are "occupied" — which only the consumer knows via metadata interpretation. |
| OQ-2 | Should there be a `Buffer.Slots.Static<M, P, let capacity: Int>` inline variant? | OPEN | Matches `Hash.Table.Static`'s `InlineArray<bucketCapacity, Int>` pattern. Lower priority — the `Fixed` heap-backed variant serves the primary consumer. |
| OQ-3 | Should metadata support `BitwiseCopyable` constraint for `memset`-based bulk fill? | OPEN | `UInt8` and `Int` are `BitwiseCopyable`, enabling `memset` for initialization. If metadata is always trivial (which it should be), this constraint could enable optimizations. |
| OQ-4 | Should `Buffer.Slots.Fixed` have `count`/`occupancy` properties? | OPEN | The buffer itself doesn't track count. But a convenience `count(where:)` method scanning metadata could be useful. This is a consumer concern, not a buffer concern. |

...
extension Buffer.Aligned {
    ...
    ///
    /// ```swift
    /// let buffer = try Buffer.Aligned.zeroed(byteCount: 4096, alignment: 4096)
    /// let header = buffer[0..<16]  // Span<UInt8> viewing the first 16 bytes
    /// ```
    ///
    ...
    public subscript(range: PartialRangeFrom<Int>) -> Span<UInt8> {
        @_lifetime(borrow self)
        borrowing get {
            bytes.extracting(range.lowerBound..<count)
        }
    }
...
//     }
//
// This API is provided by Binary.Mutable+RangeAccess.swift.

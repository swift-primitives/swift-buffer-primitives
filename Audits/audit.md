...
- **UnsafeBufferPointer typed count** (10 sites across 3 files): `Linear+Span`, `Linear.Bounded+Span`, `Linear.Inline+Span` (the conformance files were `+Memory.Contiguous` pre-dissolution; the read capability is now `Span.Protocol`)

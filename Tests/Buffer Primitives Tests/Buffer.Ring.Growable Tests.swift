...
@Suite("Buffer.Ring.Growable")
struct RingGrowableTests {

    @Test("FIFO ordering")
    func fifo() {
        var buffer = Buffer.Ring.Growable<Int>(minimumCapacity: 4)
        buffer.pushBack(10)
        buffer.pushBack(20)
        buffer.pushBack(30)
    ...
    func growth() {
        var buffer = Buffer.Ring.Growable<Int>(minimumCapacity: 2)
    ...
    func slotCapacityInvariant() {
        let buffer = Buffer.Ring.Growable<Int>(minimumCapacity: 3)
    ...
    func drain() {
        var buffer = Buffer.Ring.Growable<Int>.with([10, 20, 30])
        var drained: [Int] = []
    ...
    func removeAll() {
        var buffer = Buffer.Ring.Growable<Int>.with([1, 2, 3])
        buffer.removeAll()
        #expect(buffer.isEmpty)
    ...
    func sequenceIteration() {
        let buffer = Buffer.Ring.Growable<Int>.with([10, 20, 30])
        var collected: [Int] = []
    ...
    @Test("single element")
    func singleElement() {
        var buffer = Buffer.Ring.Growable<Int>(minimumCapacity: 1)
        buffer.pushBack(42)
    ...
    func emptyOperations() {
        let buffer = Buffer.Ring.Growable<Int>(minimumCapacity: 4)
        #expect(buffer.isEmpty)
        #expect(buffer.count == 0)
        #expect(!buffer.isFull)
    }

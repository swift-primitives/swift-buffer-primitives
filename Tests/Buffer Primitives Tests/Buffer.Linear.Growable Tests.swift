...
@Suite("Buffer.Linear.Growable")
struct LinearGrowableTests {

    @Test("append and consumeFront")
    func appendConsumeFront() {
        var buffer = Buffer.Linear.Growable<Int>(minimumCapacity: 4)
        buffer.append(10)
        buffer.append(20)
        buffer.append(30)
    ...
    func growth() {
        var buffer = Buffer.Linear.Growable<Int>(minimumCapacity: 2)
    ...
    func drain() {
        var buffer = Buffer.Linear.Growable<Int>.with([10, 20, 30])
        var drained: [Int] = []
    ...
    func removeAll() {
        var buffer = Buffer.Linear.Growable<Int>.with([1, 2, 3])
        buffer.removeAll()
        #expect(buffer.isEmpty)
    }
    ...
    func peekFrontBack() {
        let buffer = Buffer.Linear.Growable<Int>.with([10, 20, 30])
    ...
    @Test("single element")
    func singleElement() {
        var buffer = Buffer.Linear.Growable<Int>(minimumCapacity: 1)
        buffer.append(42)
